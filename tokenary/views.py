...
class PricingCatalog(BaseModel):
	...
	def from_raw(cls, raw: dict[str, Any]) -> Self:
		...
		return cls(
			sample_spec=ModelPricing.model_validate(sample_raw) if isinstance(sample_raw, dict) else None,
			models={name: ModelPricing.model_validate(data) for name, data in models.items() if isinstance(data, dict)},
		)
